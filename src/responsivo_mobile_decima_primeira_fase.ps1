...
$backupApp = Join-Path $src ("App-backup-antes-mobile-fase11-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".jsx")
$backupCss = Join-Path $src ("index-backup-antes-mobile-fase11-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".css")
...
# Substitui a navegacao inferior por botoes sem icones.
...
$marker = "/* MINI ERP MOBILE PHASE 11 */"

if ($cssCode -notmatch [regex]::Escape($marker)) {
  $phase11 = @'

/* MINI ERP MOBILE PHASE 11 */
@media (max-width: 767px) {
  ...
  .mini-mobile-drawer .mini-bottom-nav {
    display: none !important;
  }
}
'@

  Add-Content -Path $css -Value $phase11 -Encoding UTF8
}

Write-Host "Decima primeira fase mobile aplicada com sucesso." -ForegroundColor Green
Write-Host "Menu inferior sem icones aplicado: Painel, Vendas, Delivery e Tendencias." -ForegroundColor Green
