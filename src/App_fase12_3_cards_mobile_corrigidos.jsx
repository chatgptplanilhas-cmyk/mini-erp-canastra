...
Delber Vilaça`
  ...
  const itensMenu = [
    ...
    { id: 'produtos', icone: '🧀', texto: 'Produtos' },
    { id: 'produtos-controle', icone: '📦', texto: 'Produtos & Controle' },
      ...
      <section className="mobile-panel-card bg-black border border-orange-950 rounded-[28px] p-8">
        ...
        <div className="lg:hidden grid gap-3">
          ...
          {listaPendencias.map((pendencia) => (
            <article
              key={pendencia.id}
              className="rounded-2xl border border-zinc-900 bg-gradient-to-b from-zinc-950 to-black p-4 shadow-xl"
            >
              <div className="flex items-start justify-between gap-3 border-b border-zinc-900 pb-3 mb-3">
                <div className="min-w-0">
                  ...
                  <h3 className="text-lg font-black text-white break-words">
                    {pendencia.vendas?.clientes?.nome || 'Cliente não informado'}
                  </h3>
                  <p className="mt-1 text-xs text-zinc-400 break-words">
                    {pendencia.vendas?.clientes?.referencia || 'Sem referência'}
                  </p>
                </div>

                <div className="shrink-0 text-right">
                  <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-500 font-bold">Venda</p>
                  <p className="text-base font-black text-white">#{pendencia.vendas?.numero_venda || '-'}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500 font-bold">Vencimento</p>
                  <p className="font-semibold text-white">{dataBR(pendencia.vencimento)}</p>
                </div>

                <div>
                  <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500 font-bold">Status</p>
                  <span className="inline-flex rounded-xl bg-orange-950 px-3 py-1 text-xs font-bold text-orange-300">
                    {normalizarStatus(pendencia.status)}
                  </span>
                </div>

                <div>
                  <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500 font-bold">Saldo</p>
                  <p className="text-base font-black text-orange-300">{moeda(pendencia.saldo_restante)}</p>
                </div>

                <div>
                  <p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500 font-bold">Telefone</p>
                  <p className="font-semibold text-zinc-300 break-words">
                    {pendencia.vendas?.clientes?.telefone || 'Sem telefone'}
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => cobrarWhatsApp(pendencia)} className="bg-emerald-700 hover:bg-emerald-600 px-3 py-2 rounded-xl text-sm font-bold">
                  Cobrar
                </button>

                <button onClick={() => registrarPagamento(pendencia.venda_id, pendencia.saldo_restante)} className="bg-green-700 hover:bg-green-600 px-3 py-2 rounded-xl text-sm font-bold">
                  Registrar
                </button>

                <button onClick={() => confirmarPagamentoWhatsApp(pendencia)} className="bg-zinc-700 hover:bg-zinc-600 px-3 py-2 rounded-xl text-sm font-bold col-span-2">
                  Confirmar pagamento
                </button>
                ...
                <button
                  ...
                  className="bg-red-900 hover:bg-red-800 px-3 py-2 rounded-xl text-sm font-bold col-span-2"
                >
                  Excluir venda
                </button>
              </div>
